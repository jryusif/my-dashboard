...
import { SHARIAH_CONFIG, KNOWN_ISRAEL_TICKERS } from './shariah-config.js';
...
export function screenCompanyShariah({ company, financials, marketCap, previousScreening = null }) {
  ...
  // 0. Jurisdiction / Geographic Exclusion Screen
  const ticker = (company.ticker || '').toUpperCase().trim();
  const companyCountry = (company.country || '').toLowerCase().trim();
  const incCountry = (company.incCountry || '').toLowerCase().trim();
  const countryCode = (company.countryCode || '').toUpperCase().trim();
  const hqAddress = (company.hqAddress || '').toLowerCase().trim();

  const isIsraelMatch = 
    companyCountry === 'israel' || companyCountry === 'il' ||
    incCountry === 'israel' || incCountry === 'il' ||
    countryCode === 'IL' ||
    hqAddress.includes('israel') ||
    KNOWN_ISRAEL_TICKERS.has(ticker);

  const jurisdictionStatus = isIsraelMatch ? 'FAIL' : 'PASS';
  const jurisdictionFailReason = isIsraelMatch ? 'Company is domiciled, incorporated, or headquartered in Israel (Jurisdiction Exclusion)' : null;

  if (isIsraelMatch) {
    reviewReasons.push(`Jurisdiction Exclusion: ${jurisdictionFailReason}`);
    businessStatus = 'FAIL';
    businessFailReason = jurisdictionFailReason;
  }
  ...
  calculationDetails.push({
    ...
    numerator: null,
    numeratorLabel: 'Company Jurisdiction',
    denominator: null,
    denominatorLabel: 'Exclusion Policy',
    formula: 'Country / Incorporation Screening',
    result: isIsraelMatch ? 'Excluded (Israel / IL)' : 'Compliant (Non-excluded)',
    resultFormatted: isIsraelMatch ? 'Excluded (Israel / IL)' : 'Compliant (Non-excluded)',
    threshold: 'Non-excluded jurisdiction',
    thresholdFormatted: 'Non-excluded',
    status: jurisdictionStatus,
    source: company.country || (isIsraelMatch ? 'Jurisdiction Registry' : 'SEC EDGAR Submissions'),
    complianceNote: isIsraelMatch 
  ...
  if (sectorStatus === 'FAIL') {
    ...
    if (revenueImpureRatioPct > 5.0) {
      ...
      businessStatus = isIsraelMatch ? 'FAIL' : 'PASS';
      revenueStatus = 'PASS';
      revenueComplianceNote = isIsraelMatch
        ? 'Company is excluded under jurisdiction exclusion policy (Israel).'
        : '100% permissible commercial revenue from core operational contracts under AAOIFI equity governance criteria.';
    }
  }
  ...
  return {
    status,
    isExcludedJurisdiction: isIsraelMatch,
    jurisdictionStatus,
    jurisdictionReason: jurisdictionFailReason,
