...
// All periodic and annual report forms with financial statements across US domestic, foreign private issuers, and small reporting companies
export const AUTHORITATIVE_PERIODIC_FORMS = [
  '10-Q', '10-Q/A',
  '10-K', '10-K/A',
  '20-F', '20-F/A',
  '40-F', '40-F/A',
  '6-K', // Foreign issuer interim financials
  '1-SA', '1-K', '1-U'
];
...
/**
 * StockAnalysis.com Fallback — Balance Sheet Extraction (Debt, Cash, Assets)
 * 
 * Parses the SvelteKit __data.json endpoint from stockanalysis.com/stocks/{ticker}/financials/balance-sheet/
 * to extract the most recent quarterly short-term debt, long-term debt, total debt, total cash,
 * and total assets figures.
 * 
 * Used when XBRL concepts return null (e.g. foreign private issuers filing 20-F/6-K without interactive XBRL,
 * or non-standard tag filers).
 * Returns null on any network/parse error so the main flow degrades gracefully.
 */
async function getBalanceSheetFromStockAnalysis(ticker) {
  ...
  try {
    ...
    function getLatestValue(fieldName) {
      ...
      if (typeof arrIdx !== 'number') return undefined;
    ...
    function getLatestString(fieldName) {
      const arrIdx = fdMap[fieldName];
      if (typeof arrIdx !== 'number') return undefined;
      const arr = dataArr[arrIdx];
      if (!Array.isArray(arr) || arr.length === 0) return null;
    ...
    // 1. Debt
    const debtc = getLatestValue('debtc');
    const currentPortDebt = getLatestValue('currentPortDebt');
    const debtnc = getLatestValue('debtnc');
    const totalDebtSA = getLatestValue('debt');
    
    let shortTermDebt = 0;
    let longTermDebt = 0;
    let hasAnyDebt = false;
    
    if (typeof debtc === 'number') { shortTermDebt += debtc; hasAnyDebt = true; }
    if (typeof currentPortDebt === 'number') { shortTermDebt += currentPortDebt; hasAnyDebt = true; }
    if (typeof debtnc === 'number') { longTermDebt += debtnc; hasAnyDebt = true; }
    
    let totalDebt = null;
    if (hasAnyDebt) {
      totalDebt = shortTermDebt + longTermDebt;
    } else if (typeof totalDebtSA === 'number') {
      totalDebt = totalDebtSA;
    } else if (debtc === null || debtc === 0 || totalDebtSA === null || totalDebtSA === 0) {
      totalDebt = 0;
    }

    // 2. Cash & Liquid Securities
    const totalCashVal = getLatestValue('totalcash');
    const cashNeqVal = getLatestValue('cashneq');
    const cashAndSecurities = (typeof totalCashVal === 'number') 
      ? totalCashVal 
      : ((typeof cashNeqVal === 'number') ? cashNeqVal : null);

    // 3. Assets
    const assetsVal = getLatestValue('assets');
    const assetsCurrentVal = getLatestValue('assetsc');
    const totalAssets = (typeof assetsVal === 'number')
      ? assetsVal
      : ((typeof assetsCurrentVal === 'number') ? assetsCurrentVal : null);

    // 4. Fiscal Period Info
    const fiscalYear = getLatestString('fiscalYear');
    const fiscalPeriod = getLatestString('fiscalQuarter') || 'Q2';
    ...
    console.warn('[sec-provider] StockAnalysis balance sheet fallback failed for', ticker, ': ', err.message);
    return null;
  }
}

/**
 * Scrapes Market Cap and Shares Outstanding from StockAnalysis.com overview page.
 * Used as authoritative market data cross-check and fallback when SEC XBRL shares
 * are missing or underreported.
 */
export async function getStockAnalysisOverview(ticker) {
  if (!ticker) return null;
  try {
    ...
    const parseFormatted = (str) => {
      ...
      let mult = 1;
      let numPart = clean;
      if (lastChar === 'T') { mult = 1e12; numPart = clean.slice(0, -1); }
      else if (lastChar === 'B') { mult = 1e9; numPart = clean.slice(0, -1); }
      else if (lastChar === 'M') { mult = 1e6; numPart = clean.slice(0, -1); }
      else if (lastChar === 'K') { mult = 1e3; numPart = clean.slice(0, -1); }
    ...
    const mcapMatch = html.match(/Market Cap<[\s\S]*?<td[^>]*>([0-9.,]+[BMTK]?)/i);
    if (mcapMatch) marketCap = parseFormatted(mcapMatch[1]);

    const sharesMatch = html.match(/Shares Out[\s\S]*?<td[^>]*>([0-9.,]+[BMTK]?)/i);
    if (sharesMatch) sharesOut = parseFormatted(sharesMatch[1]);

    if (!marketCap && !sharesOut) return null;
    return { marketCap, sharesOut, source: 'StockAnalysis.com' };
  } catch (err) {
    return null;
  }
}
...
export async function getCompanySubmissions(cik) {
  ...
  for (let i = 0; i < totalCount; i++) {
    ...
    if (form && AUTHORITATIVE_PERIODIC_FORMS.includes(form)) {
      periodicFilings.push(item);
    }
  }

  // Sort periodic filings prioritizing newest filingDate descending, then reportDate descending
...
export async function getCompanyFinancialFacts(cik, targetAccn = null, ticker = null) {
  ...
  // Universal: Scan all available XBRL filings to discover the absolute latest reporting period and filing
  const xbrlFilingsMap = new Map();
  for (const [cName, cNode] of Object.entries(gaap)) {
    const units = cNode.units?.USD || cNode.units?.shares || Object.values(cNode.units || {})[0];
    if (!Array.isArray(units)) continue;
    for (const u of units) {
      if (!u.form || !AUTHORITATIVE_PERIODIC_FORMS.includes(u.form)) continue;
      if (!xbrlFilingsMap.has(u.accn)) {
        xbrlFilingsMap.set(u.accn, {
          form: u.form,
          filed: u.filed,
          end: u.end,
          fp: u.fp,
        ...
        const item = xbrlFilingsMap.get(u.accn);
        if (u.end && (!item.end || u.end > item.end)) item.end = u.end;
        if (u.filed && (!item.filed || u.filed > item.filed)) item.filed = u.filed;
      }
    }
  }

  // Sort available XBRL filings prioritizing latest reporting period end date descending, then filed date descending
  const sortedXbrlFilings = [...xbrlFilingsMap.values()].sort((a, b) => {
    const endA = a.end || '';
    const endB = b.end || '';
  ...
  const absoluteLatestXbrlFiling = sortedXbrlFilings[0] || null;

  // Determine effective target accession number:
  // If targetAccn is provided AND present in XBRL facts with financial data, use it.
  // Otherwise, automatically target the absolute newest filing found in XBRL company facts.
  const effectiveTargetAccn = (targetAccn && xbrlFilingsMap.has(targetAccn))
    ? targetAccn
    : (absoluteLatestXbrlFiling?.accn || targetAccn);
  ...
  function extractConceptLatest(conceptNames) {
    ...
    for (const name of conceptNames) {
      ...
      // Filter for authoritative periodic filings
      const periodicItems = units.filter(u => u.form && AUTHORITATIVE_PERIODIC_FORMS.includes(u.form));
      const candidates = periodicItems.length > 0 ? periodicItems : units;

      // If effective targetAccn is specified, prioritize the exact matching filing accession number
      if (effectiveTargetAccn) {
        const matchingAccn = candidates.filter(u => u.accn === effectiveTargetAccn);
        if (matchingAccn.length > 0) {
          // CRITICAL: Sort by reporting period end date descending so we NEVER pick comparative prior years!
          matchingAccn.sort((a, b) => (b.end || '').localeCompare(a.end || ''));
          return {
            concept: name,
      ...
      // Universal fallback across all candidates:
      // Sort by reporting period end date descending, then filed date descending
  ...
  // —— StockAnalysis.com Fallback for Balance Sheet (Debt, Cash, Assets) ——
  // When XBRL tags return null (foreign issuers on 6-K/20-F without interactive XBRL, or untagged filers),
  // or when available XBRL facts are severely stale (>180 days), try StockAnalysis as authoritative fallback.
  let debtFallbackSource = null;
  let saFallback = null;
  if (ticker) {
    try {
      saFallback = await getBalanceSheetFromStockAnalysis(ticker);
    } catch (_) { /* fallback failed silently */ }
  }

  const isXbrlCashStale = !cashFact || (cashFact.end && (Date.now() - new Date(cashFact.end).getTime()) > (180 * 24 * 60 * 60 * 1000));
  const isXbrlDebtStale = !hasDebtData || (!shortDebtFact && !longDebtFact && !combinedDebtFact);
  const isXbrlAssetsStale = !assetsFact || (assetsFact.end && (Date.now() - new Date(assetsFact.end).getTime()) > (180 * 24 * 60 * 60 * 1000));

  if (saFallback) {
    if (isXbrlDebtStale && saFallback.totalDebt !== null) {
      totalDebtVal = saFallback.totalDebt;
      hasDebtData = true;
      debtFallbackSource = saFallback.source;
      console.log('[sec-provider] Using SA fallback debt for', ticker, ':', saFallback.totalDebt, 'from', saFallback.source);
    }
    if ((!hasCashData || isXbrlCashStale) && saFallback.cashAndSecurities !== null) {
      cashSecuritiesVal = saFallback.cashAndSecurities;
      hasCashData = true;
      console.log('[sec-provider] Using SA fallback cash for', ticker, ':', saFallback.cashAndSecurities, 'from', saFallback.source);
    }
  }
  ...
  function extractSharesOutstanding() {
    function getLatestFromNode(conceptNode) {
      if (!conceptNode || !conceptNode.units) return null;
      const units = conceptNode.units.shares || Object.values(conceptNode.units)[0];
      if (!Array.isArray(units) || units.length === 0) return null;

      const valid = units.filter(u => typeof u.val === 'number' && u.val > 0);
      if (valid.length === 0) return null;

      // Sort prioritizing latest financial balance sheet period (end), then filing date (filed)
      ...
      if (effectiveTargetAccn) {
        const match = sorted.filter(u => u.accn === effectiveTargetAccn);
    ...
    const deiShares = getLatestFromNode(dei.EntityCommonStockSharesOutstanding);
    if (deiShares) return deiShares;

    // 2. GAAP CommonStockSharesOutstanding
    const gaapCommon = getLatestFromNode(gaap.CommonStockSharesOutstanding);
    if (gaapCommon) return gaapCommon;

    // 3. GAAP SharesOutstanding
    const gaapShares = getLatestFromNode(gaap.SharesOutstanding);
    if (gaapShares) return gaapShares;

    // 4. GAAP CommonStockSharesIssued
    const gaapIssued = getLatestFromNode(gaap.CommonStockSharesIssued);
    if (gaapIssued) return gaapIssued;

    // 5. GAAP WeightedAverageNumberOfDilutedSharesOutstanding
    const dilutedShares = getLatestFromNode(gaap.WeightedAverageNumberOfDilutedSharesOutstanding);
    if (dilutedShares) return dilutedShares;

    // 6. GAAP WeightedAverageNumberOfSharesOutstandingBasic
    const basicShares = getLatestFromNode(gaap.WeightedAverageNumberOfSharesOutstandingBasic);
    if (basicShares) return basicShares;
  ...
  function extractPublicFloat() {
    ...
    if (floatNode && floatNode.units) {
      ...
      if (Array.isArray(units) && units.length > 0) {
        ...
        if (sorted.length > 0) return sorted[0].val;
      }
    }
    return null;
  }

  let sharesOutstandingVal = extractSharesOutstanding();
  let publicFloatUsdVal = extractPublicFloat();

  // Cross-check with StockAnalysis overview if ticker provided and shares appear missing or underreported
  if (ticker && (!sharesOutstandingVal || (publicFloatUsdVal && sharesOutstandingVal < 2000000 && publicFloatUsdVal >= 5000000))) {
    try {
      const saOverview = await getStockAnalysisOverview(ticker);
      if (saOverview?.sharesOut && saOverview.sharesOut > (sharesOutstandingVal || 0)) {
        sharesOutstandingVal = saOverview.sharesOut;
      }
    } catch (_) {}
  }

  // Determine latest reporting period info by taking the fact with the newest reporting period end date
  const allExtractedFacts = [assetsFact, revenueFact, cashFact, shortDebtFact, longDebtFact, combinedDebtFact].filter(Boolean);
  allExtractedFacts.sort((a, b) => {
    const endA = a.end || a.filed || '';
    const endB = b.end || b.filed || '';
    if (endB !== endA) return endB.localeCompare(endA);
    return (b.filed || '').localeCompare(a.filed || '');
  });

  const primaryPeriod = (!isXbrlAssetsStale && allExtractedFacts[0]) || (saFallback?.fiscalYear ? {
    form: '6-K',
    fy: saFallback.fiscalYear,
    fp: saFallback.fiscalPeriod || 'Q2',
    end: null,
    filed: null,
    accn: null
  } : (allExtractedFacts[0] || absoluteLatestXbrlFiling || null));

  // Compute Revenue Segment Breakdown (AAOIFI Standard No. 21 Rule 1 & 2)
  const revenueBreakdown = extractRevenueSegmentBreakdown(gaap, effectiveTargetAccn, revenueFact, extractConceptLatest);
  ...
  return {
    ...
    metrics: {
      totalAssets: (assetsFact && !isXbrlAssetsStale) ? assetsFact.val : (saFallback?.totalAssets || assetsFact?.val || null),
      ...
      // Under US GAAP, if no interest income is reported and revenue is positive, interest income is $0.
      interestIncome: (() => {
        const rev = revenueFact?.val;
        if (!interestIncomeFact) {
          return (typeof rev === 'number' && rev > 0) ? 0 : null;
        }
        const raw = interestIncomeFact.val;
        if (typeof raw !== 'number') return (typeof rev === 'number' && rev > 0) ? 0 : null;
      ...
      impureIncome: (() => {
        ...
        if (typeof raw !== 'number') return (typeof rev === 'number' && rev > 0) ? 0 : null;
        const clamped = Math.max(0, raw);
        const revVal = revenueFact?.val;
        if (typeof revVal === 'number' && revVal > 0 && clamped > revVal) return null;
        return clamped;
      })(),
      revenueBreakdown,
